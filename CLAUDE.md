...
- **Entry points**: `core/ssm/env.sh`, `core/build/run.sh`, `core/leak/check.sh`, `core/s3/publish.sh`, `core/cloudfront/invalidate.sh` — invoked in that order by `action.yml` composite steps; `core/` is split by domain (`ssm/` from action-ssm-env-build, `s3/` from action-s3-publish, `cloudfront/` from action-cloudfront-publish, `build/` new, `leak/` new)
...
| `core/leak/check.sh` | Fails the build when a `SecureString` parameter value (16+ chars) appears in the build output |
...
| `tests/leak.bats` | BATS tests for `core/leak/check.sh` |
