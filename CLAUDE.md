...
- **Entry points**: `core/ssm/env.sh`, `core/build/run.sh`, `core/s3/publish.sh` — invoked in that order by `action.yml` composite steps; `core/` is split by domain (`ssm/` from action-ssm-env-build, `s3/` from action-s3-publish, `build/` new)
